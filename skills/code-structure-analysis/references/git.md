...
**Prefer `git grep` over `grep -r` for large codebases** — git grep is faster, respects .gitignore, and avoids timeouts.

```bash
git grep "pattern"                    # Search all tracked files
git grep "pattern" -- path/           # Limit to path
git grep -n "pattern"                 # Show line numbers
git grep -l "pattern"                 # List files only
...
- Use the `gh` CLI for GitHub-specific operations
- Use the `/git-workflow` skill for advanced Git workflows
- Use the `/service-migration` skill for migration workflows
