...
Day-to-day idioms for Swift's modern concurrency (async/await, actors, structured concurrency).

**Scope split (read first):**
- This file — runtime patterns: when to reach for `actor`, `@MainActor`, `async let`, `TaskGroup`, cancellation.
- [`strict-concurrency-migration.md`](strict-concurrency-migration.md) — Swift 6 language-mode migration mechanics: `Sendable`, `@preconcurrency`, `nonisolated(unsafe)`, `MainActor.assumeIsolated`.
- [`approachable-concurrency.md`](approachable-concurrency.md) — Swift 6.2 main-actor-default + `NonisolatedNonsendingByDefault`.

In Swift 6 language mode, data-race safety is a **compile-time error**, not a warning. Most patterns below stay the same; the compiler just stops being lenient about gaps. Source: [Apple — Swift Concurrency](https://developer.apple.com/documentation/swift/concurrency) · [WWDC25 "Embracing Swift concurrency" (268)](https://developer.apple.com/videos/play/wwdc2025/268/).
...
- [iOS Best Practices](../ios/ios-best-practices.md) - iOS-specific patterns
- [Style Guide](style-guide.md) - Swift style conventions

---

## Swift 6 Strict Concurrency

Swift 6 makes data-race safety a compile-time error rather than a warning. The migration is **incremental and per-module** — see [`strict-concurrency-migration.md`](strict-concurrency-migration.md) for the full playbook (warnings-first, `Sendable`, `@preconcurrency`, `nonisolated(unsafe)`, `MainActor.assumeIsolated`). Swift 6.2 adds an opt-in "approachable concurrency" mode (main-actor-default) covered in [`approachable-concurrency.md`](approachable-concurrency.md).

Quick rules of thumb under Swift 6:
- Value types are `Sendable` for free if all stored properties are `Sendable`.
- `@unchecked Sendable` is a programmer assertion — only valid when you can name the synchronization mechanism (lock, serial queue, atomics).
- Annotate the **type** with `@MainActor`, not every method — isolation is transitive.
- Bridge legacy SDKs with `@preconcurrency import` rather than disabling the checker.

---

## References

- [Swift Concurrency — Apple](https://developer.apple.com/documentation/swift/concurrency)
...
- [WWDC24 "Migrate your app to Swift 6" (10169)](https://developer.apple.com/videos/play/wwdc2024/10169/)
