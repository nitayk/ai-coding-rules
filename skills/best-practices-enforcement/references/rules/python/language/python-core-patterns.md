...
**Preferred:** `d1 | d2` or `d1 |= d2` for merging dicts. Avoid `{**d1, **d2}` when the merge operator is available.
...
Use `/` and `*` in signatures to future-proof APIs and enforce intended usage: `def f(a, b, /, c, *, d)` – `a,b` positional-only, `d` keyword-only. Helps library evolution and refactoring.
