...
**When threading helps (despite the GIL):** The GIL is released on blocking I/O (sockets, file ops) and in C extensions. Use threads for: (1) **Application responsiveness** – keep UI responsive while doing background work; (2) **Multiuser/network apps** – serve many connections; (3) **Work delegation** – parallel HTTP requests, external API calls, file indexing. For CPU-bound pure-Python code, threads do not speed things up; use multiprocessing instead. **Hybrid:** Multiple processes with multiple threads each (e.g., Gunicorn gthread workers) can utilize multicore + I/O parallelism.
...
**Memory profiling:** Python does not release memory eagerly. Use `memory_profiler` or `objgraph` for memory leaks. `objgraph.show_refs()` / `show_backrefs()` help visualize object retention.
