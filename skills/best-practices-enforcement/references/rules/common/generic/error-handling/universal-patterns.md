...
**DO**:
- Check error returns right after the call—don't defer
- Handle or propagate before using the result
- Use guard clauses to exit early on error

**STOP**:
- Deferring error checks (risk of using invalid result)
- Ignoring errors with blank identifier (`_`) or empty catch
- Continuing execution after a failed operation
...
// ❌ Bad: Deferring check
result, err := doSomething()
processResult(result)  // result may be invalid!
if err != nil {
    return nil, err
}
```
