...
## Lazy Programmer: Effort for Maximal Gain

**Core Principle**: **Spend effort only where it yields maximal gain.** Minimize friction; as friction halves, possibilities multiply.

**Origin**: Larry Wall (Perl, 1991) — "Laziness" as a programmer virtue: *"The quality that makes you go to great effort to reduce overall energy expenditure. It makes you write labor-saving programs that other people will find useful."* Jeremy Bowers (jerf.org): *"At all times, effort spent must be for the maximal gain."*

**DO**:
- Reduce friction (tooling, abstractions, reuse) so more becomes feasible
- Think before coding — "a moment spent thinking can literally be worth months spent coding"
- Make small, standalone components — easier to recombine when requirements change
- Automate repetitive work; document so you don't re-explain
- Solve problems correctly the first time — temporary fixes create future work

**STOP**:
- Brute-force solutions when a simpler general solution exists
- Over-engineering for hypothetical future needs
- Copy-pasting instead of extracting reusable components

**Friction insight**: As friction halves, possibilities double. Low friction changes not only what you *can* do but what you *consider* doing.
...
- Prefer a built-in **platform/runtime primitive over adding a third-party dependency** — e.g. an HTML input type (`<input type="date">`) over a date-picker library, CSS over a JS animation lib, a DB constraint over app-level validation, `Intl`/`URL`/stdlib date APIs over a formatting package. Reach for a dependency only when the platform genuinely doesn't cover the need.
- Write code that junior developers can understand

**STOP**:
- Over-engineering for hypothetical future needs
- Reaching for a new dependency when a platform primitive already does the job (the most expensive line of code is the one you didn't need to write — or install)
...
**Reference**: For detailed examples and property-based testing patterns, see language-specific rules (e.g., `backend/scala/meta/correctness-first.md`).
...
**Reference**: For comprehensive examples with sealed traits, ADTs, and AI-safe patterns, see language-specific rules (e.g., `backend/scala/language/make-illegal-states-unrepresentable.md`).

---

## Pure Functions & Referential Transparency

**Core Principle**: Write functions that are **referentially transparent** - same input always produces same output, no side effects. An expression can be replaced with its value without changing program behavior.

**Origin**: John Hughes, "Why Functional Programming Matters" (1989) — higher-order functions and lazy evaluation contribute greatly to modularity. Chalmers 2015 review: FP changes how programs are constructed, verified, and reasoned about.
...
- **Eliminates entire classes of bugs** — no assignment/side effects = fewer failure modes
- **Enables equational reasoning** — replace expression with value; proof-friendly
- **Prevents nondeterminism** — concurrent systems: pure functions avoid race conditions
- **Property-based testing** — automates edge-case discovery; higher confidence than unit tests alone
- **Modularity** — Hughes: "Conventional languages place conceptual limits on modularisation; functional languages push those limits back"

**Pattern**: Functional core, imperative shell - extract pure business logic from side-effecting code.

**Reference**: For detailed patterns on encoding errors in types, separating pure functions, and functional composition, see language-specific rules (e.g., `backend/scala/language/referential-transparency.md`, `backend/scala/meta/functional-programming-principles.md`).
...
- [Testing Principles](../testing/core-principles.md) - TDD, test behavior not implementation
