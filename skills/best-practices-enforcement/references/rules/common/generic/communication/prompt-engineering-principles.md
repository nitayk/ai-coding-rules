...
**📖 LOAD MANUALLY**: This rule does NOT auto-load. Reference with `@prompt-engineering-principles.md` or `@generic/communication/prompt-engineering-principles.md` when needed.
...
**Example**:
```
✅ Simple: Repeat prompt twice for retrieval tasks
❌ Complex: Multi-shot prompting with examples when simple repetition works
```
...
**Cost Tradeoff**:
- ✅ Doubles input tokens (cost)
- ✅ Minimal latency impact (prefill is parallelizable)
- ✅ Large accuracy gains (up to 76% improvement)
- ✅ No increase in output tokens
...
### ✅ Good: Task-Appropriate Technique
...
Ask for output "from the perspective of a staff engineer" to get more structured, senior-level docs and plans:

```
Write this design doc from the perspective of a staff engineer. Focus on tradeoffs, constraints, and maintainability.
```

**When**: Design docs, architecture decisions, implementation plans. Raises quality of structure and depth.

### Refactor for Brevity

Tame messy or verbose agentic code with explicit brevity constraints:

```
Refactor this script to be shorter, more elegant, and less fluff—but keep it readable. Remove redundant comments and over-engineering.
```

**When**: After AI-generated code that is verbose, over-commented, or over-abstracted. Use with `/code-cleanup` for systematic cleanup.

---

## Related Rules

- [Tool Communication Pattern](tool-communication-pattern.md) - How to explain tools when using them
- [Business Communication Standards](business-communication-standards.md) - BLUF format for explanations
- [Performance Principles](../performance/core-principles.md) - Measure first, optimize bottlenecks
- [Code Quality Principles](../code-quality/core-principles.md) - Simple solutions first
