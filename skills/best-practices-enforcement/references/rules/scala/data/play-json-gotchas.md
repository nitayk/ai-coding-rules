...
**Scala-Specific:**
- [JSON Serialization Patterns](json-serialization-patterns.md) - General Play JSON patterns
...
```scala
// Bad: WRONG - orElse NEVER triggers because readNullable returns JsSuccess(None)
...
**Correct pattern:**
```scala
// Good: CORRECT - read both fields separately, combine with Scala orElse
for {
...
// Format A: Object
// {"provider": "providerA", "instanceId": 123}
...
```scala
// Repo A: "ProviderA" (PascalCase)
// Repo B: "providerA" (camelCase)
```
...
private def toExpectedFormat(provider: Provider): String = provider match {
  case Provider.ProviderA => "ProviderA"    // Force specific casing
