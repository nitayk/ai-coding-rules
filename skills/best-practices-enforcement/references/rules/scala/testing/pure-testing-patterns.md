...
**Note**: For **production code**, prefer **named traits** (SAM types) over anonymous function types for better discoverability and tooling support. Anonymous functions are fine for **tests**. See [Avoid Anonymous Function Dependencies](../language/avoid-anonymous-function-dependencies.md) for details.
...
class DataProcessorSpec extends AnyFlatSpec with Matchers {
  ...
  // Good: Also fine: Anonymous functions in tests for simplicity
...
For simple cases, inline anonymous functions make tests even more concise. **Note**: Anonymous functions are fine in tests, but production code should use named traits. See [Avoid Anonymous Function Dependencies](../language/avoid-anonymous-function-dependencies.md).
...
class DataProcessorSpec extends AnyFlatSpec with Matchers {
  ...
  // Good: Also fine: Lambda syntax for very simple cases in tests
  it should "reject invalid records" in {
    val processor = new DataProcessor(
      validator = new DataValidator { def validate(_: DataRecord) = false },
      repository = new DataRepository { def save(_: DataRecord) = Success(RecordId("123")) }
...
class OrderServiceSpec extends AnyFlatSpec with Matchers {
  ...
  // Good: Better: Use Given-When-Then structure
...
class OrderProcessorSpec extends AnyFlatSpec with Matchers {
  ...
  // Good: Better: Even more declarative with test cases as data
  val testCases = List(
    TestCase(
...
class OrderProcessorSpec extends AnyFlatSpec with Matchers {
  ...
  // Good: Better: Declarative test cases
  val testCases = List(
    (
      "valid order",
      withOrder(Order(items = List(Item("book", 10.0)))) |+|
...
// Good: Better: Use test data builders with defaults
object TestOrders {
...
- [Generic Testing Principles](../../../../generic/testing/core-principles.md) - Universal testing principles (test behavior not implementation, AAA pattern, isolation, pure testing patterns)

**Scala-Specific:**
- [Avoid Anonymous Function Dependencies](../language/avoid-anonymous-function-dependencies.md) - **Use named traits for production dependencies** - Anonymous functions are fine in tests
- [Referential Transparency](../language/referential-transparency.md) - Pure functions and side-effects
- [Compiler-Friendly Types](../language/compiler-friendly-types.md) - Type safety in tests
- [Functional Composition and HOF](../language/functional-composition-and-hof.md) - Composing test utilities
- [Scala Testing Best Practices](scala-testing-best-practices.md) - General testing practices
