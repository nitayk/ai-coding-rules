...
**Important**: Always **[make it correct first](../meta/correctness-first.md)** before optimizing. Correctness is the foundation - optimize only after correctness is verified with comprehensive tests.

---

- **Distinguish between cold paths (startup/configuration) and hot paths (serving/computation)**:  
  ```scala
  // Good: Cold path: Use immutable, functional patterns freely
  object AppConfig {
  ...
  // Good: Hot path: Optimize for performance, prefer simple operations
...
- **Prefer allocation-light patterns in hot paths**:  
  ...
  // Bad: Avoid in hot paths: Creates intermediate collections
...
- **Use lazy evaluation strategically**:  
  ...
  **See [Lazy Evaluation and Productivity](../language/lazy-evaluation-and-productivity.md)** for comprehensive guidance on working with infinite sequences and understanding productivity.
...
- **Use context-appropriate effect management**:  
  ...
  // Bad: Avoid: Mixing effect systems unnecessarily
  def mixedEffects(id: String): Future[ZIO[Any, Error, User]] = ???
  ```

- **Consider mutability for builders and accumulators**:  
  ```scala
  // Good: When building large collections, mutable builders can be more efficient
...
**Scala-Specific:**
- [Correctness First](../meta/correctness-first.md) - **Make it correct first, then optimize** - Correctness is the foundation
- [Lazy Evaluation and Productivity](../language/lazy-evaluation-and-productivity.md) - Understanding when functions are productive and work with infinite sequences
