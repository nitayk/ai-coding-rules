...
### 1. **Optimize for Readability, Not File Count** 🎯

File count doesn't matter - **readability does!**

```scala
// Bad: Everything in one 1,500 line file
object RVGatewayRoute {
  // 598 lines of debug routes + 400 lines of main routes + ...
  // = "where is what I need??"
}

// Good: Separate by concern (even if more files)
routes/
  ├── RVGatewayRoutes.scala       (150 LOC) - Main routes
  ├── RVGatewayDebugRoutes.scala  (200 LOC) - Debug routes  
  └── RouteParams.scala           (50 LOC)  - DTOs
```

### 2. **Hide Implementation Details in Modules** 📦
...
### 3. **Small Files Are OK (Even Encouraged!)**
...
### 4. **Package Structure Mirrors Features, Not Layers** 🗂️

```scala
// Good: Feature-oriented
gateway/
  ├── skan/              // All SKAN code here
  ├── callbacks/         // All callback code here
  └── attribution/       // All attribution code here
...
```scala
// Good: Case classes provide immutability and structure
...
// Good: Type-safe Duration for time values
...
// Good: Duration literals are readable and type-safe
val timeout = 5.seconds
val retryDelay = 250.milliseconds
val maxWait = 10.minutes

// Bad: Raw Long values without context
...
// Bad: Magic numbers without clear units
Thread.sleep(5000) // 5000 what? Milliseconds? Seconds?
```

### Duration JSON Serialization
```scala
// Good: Custom JSON formatting for Duration types
...
### Duration Benefits
```scala
// Good: Type Safety: Compile-time unit verification
...
// Good: Comparison: Natural comparison operators
...
```scala
// Good: PascalCase for classes, objects, traits
case object JsonProcessor extends DataProcessor
trait EventHandler
class ConfigurationManager

// Good: camelCase for methods and values
...
### Descriptive Names
```scala
// Good: Names that describe purpose
...
```scala
// Good: Singular case class for ONE item
case class RecordNameInput(recordName: String, alias: Option[String])

// Good: Plural object for COLLECTION operations
...
// Bad: Plural case class name for singular item
case class RecordNamesInput(recordName: String, alias: Option[String])

// Bad: Singular object name for collection operations
...
```scala
// Good: Use maybe prefix for Option types
...
// Bad: Mixed responsibilities in same object
object RecordNameInput {
...
```scala
// Good: Expression-based control flow
...
```scala
// Good: Immutable collections by default
...
```scala
// Good: Preserve exact behavior during refactoring
...
// Bad: Large refactoring without intermediate compilation
...
```scala
// Good: Variable defined right before usage
...
// Bad: Variable defined far from usage
def processData(config: Config): DataFrame = {
...
// 3. New tests verify preserved behavior
...
// Good: Composing error-prone operations
for {
...
```scala
// Good: Pass only the required 'name' field
...
// Bad: Pass the entire object when only one field is needed
...
```scala
// Good: Logger passed implicitly through the call chain
...
- `Monitoring` - cross-cutting metric collection
- `Logger` / `AppLogger` - cross-cutting logging
- `TimeProvider` - consistent time source
- `ExecutionContext` - async execution
- `SparkSession` - Spark operations
- Genuinely global configuration contexts (e.g., `ABFeatureConfig`, `RvGatewayConfig`)

**NEVER use implicit for business data or request-specific context!**

```scala
// Good: Only cross-cutting concerns implicit
def buildClickTags(
  ...
  logger: AppLogger,                        // IMPLICIT: cross-cutting
  config: RvGatewayConfig                   // IMPLICIT: global config
): Option[ClickTags]

// Bad: Business data passed implicitly (REAL EXAMPLE from Gateway!)
def build(
  callbackUrlData: RequestCallbackUrlData,
  gatewayExternalData: Option[GatewayExternalData],
  // ... 10+ explicit params ...
)(implicit 
  requestUri: EncodedUri,              // Bad: Request data, NOT cross-cutting!
  rvServeData: RVServeData,            // Bad: Business data, NOT cross-cutting!
  appAugmentingData: AppAugmentingData, // Bad: Business data, NOT cross-cutting!
  rvRequest: RVRequest,                 // Bad: Request data, NOT cross-cutting!
  displayRequestID: DisplayRequestID,   // Bad: Request context, NOT cross-cutting!
  requestId: RequestId,                     // Bad: Request context
  platformType: PlatformType,           // Bad: Request context
  remoteAddressOption: Option[InetAddress], // Bad: Request data
  userAgent: Option[UserAgentWrapper],  // Bad: Request data
  translations: Map[String, String],    // Bad: Config/data
...
```scala
// Good: Consistent implicit parameter patterns
...
// Bad: Mixing implicit and explicit patterns inconsistently
...
```scala
// Good: LoggingUtil with implicit logger and Kibana-friendly tags
...
```scala
// Good: Logger and similar tools passed implicitly  
...
```scala
// Good: Implicit parameters at the end
def processTimestamp(df: DataFrame, 
                     timestampColumn: Option[String], 
                     mode: TimestampMode)
                    (implicit logger: Logger): DataFrame = { ... }

// Good: Multiple implicit parameter groups
...
// Bad: Implicit parameters mixed with regular parameters
def processTimestamp(df: DataFrame, 
                     timestampColumn: Option[String])
...
For detailed pattern matching guidelines, see the dedicated [Pattern Matching Best Practices](pattern-matching-best-practices.md) rule file.
...
### Quick Reference
```scala
// Good: Bind objects and access fields
...
// Bad: Excessive destructuring with underscores
def processTableIdentifier(tableId: TableIdentifier): String = tableId match {
...
// Good: Type-safe duration with clear units
...
// Bad: Raw Long values lose type information
case class TimerConfig(
...
```scala
// Good: Keep FiniteDuration internally, convert at logging boundary
...
// Bad: Provide *Ms helper methods that lose type safety
...
```scala
// Good: Natural duration arithmetic
...
// Bad: Manual millisecond calculations
...
```scala
// Good: JSON serialization for FiniteDuration
implicit val durationFormat: Format[FiniteDuration] = Format[FiniteDuration](
...
- [Generic Code Quality Principles](../../../generic/code-quality/core-principles.md) - Universal principles (meaningful names, code organization, comments, Boy Scout Rule)
