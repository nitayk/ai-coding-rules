...
```scala
// Bad: Bad: Illegal states are representable
...
// Bad: Problem: Can create invalid combinations
...
```scala
// Bad: Bad: AI replaces hardcoded default with dynamic lookup
case class CoffeeOrder(
  drinkType: DrinkType,
...
// Bad: Problem: What if stockService returns null or throws?
...
```scala
// Good: Good: Each variant represents only valid combinations
...
// Good: Impossible to create invalid combinations
val validIrishCoffee = IrishCoffee(
  cream = Cream.Fresh,
  whiskey = Whiskey.JAMESON
)  // Compiler ensures both are provided

// Bad: Compiler error: Can't create IrishCoffee without whiskey
...
```scala
// Good: Good: Types reflect optionality and failure
...
// Good: Type system forces handling all cases
...
```scala
// Bad: Bad: Invalid combinations possible
case class Policy(
...
// Good: Good: Sealed traits enforce valid combinations
sealed trait Policy
...
```scala
// Bad: Bad: Nullable fields allow invalid combinations
...
```scala
// Bad: Bad: Mutable state allows invalid transitions
case class Order(
  status: OrderStatus,
...
// Good: Good: Sealed traits enforce valid state transitions
sealed trait Order
...
```scala
// Good: Good: Sealed trait with case classes for each valid variant
...
// Good: Good: NonEmptyList guarantees at least one element
case class RestrictedPolicy(
  availabilityZones: NonEmptyList[AvailabilityZone],
  maxInstances: Int
)
...
```scala
// Good: Good: Typed wrappers prevent mixing up values
...
```scala
// Good: Good: Either makes failure modes explicit
...
```scala
// Bad: Bad: AI might break invariants
case class CoffeeOrder(
  drinkType: DrinkType,
...
// Bad: Problem: Now can create IrishCoffee without whiskey!
...
```scala
// Good: Good: Type system prevents invalid edits
sealed trait CoffeeOrder
case class IrishCoffee(cream: Cream, whiskey: Whiskey) extends CoffeeOrder
...
```scala
// Good: Good: Exhaustive pattern matching
...
```scala
// Good: Good: Property-based tests verify invariants
...
```scala
// Bad: Bad: Empty list still possible
case class RestrictedPolicy(availabilityZones: List[AvailabilityZone])

// Good: Good: NonEmptyList guarantees at least one
...
```scala
// Bad: Bad: Single type allows invalid combinations
case class Policy(
  isRestricted: Boolean,
...
// Good: Good: Separate types for valid combinations
sealed trait Policy
...
- [Generic Code Quality Principles](../../../../generic/code-quality/core-principles.md) - Universal "make illegal states unrepresentable" principle (this file provides Scala-specific implementation with sealed traits)

**Scala-Specific:**
- [Compiler-Friendly Types](compiler-friendly-types.md) - Foundation for type safety
- [Error Handling Patterns](error-handling-patterns.md) - Using `Either` for explicit failure modes
- [Avoid Option Blindness](avoid-option-blindness.md) - When `Option` loses domain semantics
- [Prefer Case Classes Over Tuples](prefer-case-classes-over-tuples.md) - Structured data with named fields
- [Correctness First](../meta/correctness-first.md) - Make it correct first, then optimize
